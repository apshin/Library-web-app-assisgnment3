...
function BookList({ loggedIn, guestMode, loggedInUser }) {
    ...
    const handleConfirmChapters = () => {
        const chapters = chapterInputs.slice(0, parseInt(numChapters));
        setChapterInputs(chapters);
    ...
    const handleChapterTitleChange = (index, e) => {
        const updatedChapterInputs = [...chapterInputs];
        updatedChapterInputs[index] = { ...updatedChapterInputs[index], title: e.target.value };
        setChapterInputs(updatedChapterInputs);
    };

    const handleChapterContentChange = (index, e) => {
        const updatedChapterInputs = [...chapterInputs];
        updatedChapterInputs[index] = { ...updatedChapterInputs[index], content: e.target.value };
        setChapterInputs(updatedChapterInputs);
    };

    // Render input fields for chapter titles and content based on numChapters
    const renderChapterInputs = () => {
        return chapterInputs.map((chapter, index) => (
            <div key={index}>
                <Form.Group controlId={`formChapterTitle${index + 1}`}>
                    <Form.Label>Chapter {index + 1} Title</Form.Label>
                    <Form.Control type="text" placeholder={`Enter title for Chapter ${index + 1}`} value={chapter?.title || ''} onChange={(e) => handleChapterTitleChange(index, e)} />
                </Form.Group>
                <Form.Group controlId={`formChapterContent${index + 1}`}>
                    <Form.Label>Chapter {index + 1} Content</Form.Label>
                    <Form.Control as="textarea" rows={3} placeholder={`Enter content for Chapter ${index + 1}`} value={chapter?.content || ''} onChange={(e) => handleChapterContentChange(index, e)} />
                </Form.Group>
            </div>
        ));
    };
    ...
    return (
        <div className='book-list'>
            ...
            <div className='booklist'>
                ...
                <Modal show={showAddChaptersModal} onHide={() => setShowAddChaptersModal(false)}>
                    <Modal.Header closeButton>
                    ...
                    <Modal.Body>
                        ...
                        <Form.Group controlId="formNumChapters">
                            ...
                            <Form.Control type="text" value={numChapters} placeholder="Please Enter Number Of Chapters" onChange={handleNumChaptersChange} />
                        </Form.Group>
                        {renderChapterInputs()}
                    </Modal.Body>
                    <Modal.Footer>
                        <Button variant="secondary" className="button-style" onClick={() => setShowAddChaptersModal(false)}>Cancel</Button>
                        <Button variant="primary" className="button-style" onClick={handleConfirmChapters}>Confirm</Button>
                    </Modal.Footer>
                </Modal>
                {!loading && (
